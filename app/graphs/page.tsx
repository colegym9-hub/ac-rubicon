...
export default async function GraphsPage() {
  const { configured, days, throughput, adherence, trends } = await getInsights();
    ...
    <main className="mx-auto flex min-h-dvh w-full max-w-md md:max-w-none flex-col gap-6 px-5 md:px-10 pt-10 pb-24 md:pb-10">
      ...
      {!configured ? (
        ...
        >
          ...
          <code>.env.local</code> — charts fill in as you use the app.
        </div>
      ) : null}

      {empty ? (
        <p className="text-sm text-muted-foreground">
          No history yet. Complete tasks, plan days, and log metrics — the charts
          below fill in over the last 14 days.
        </p>
      ) : null}
      ...
      <p className="font-mono text-[0.6rem] uppercase leading-relaxed tracking-[0.15em] text-muted-foreground">
        ...
        (depends on the scheduler runtime decision).
      </p>
    </main>
  );
}
