...
- [ ] Add brain tools to `lib/mcp/tools.ts` (register alongside existing tools): `get_pending_work`, `convert_source` (calls Supadata `/transcript?url=`, handles async job-id polling), `ingest_source` (raw source → wiki page per `Cole-2nd-Brain/CLAUDE.md` SOP), `get_wiki_page`, `search_wiki` (FTS query), `upsert_wiki_page`, `append_brain_log`, `insert_raw_source`, `get_brain_pages`, `save_lint_report`, `save_insight`. All read or safe-write only (no delete tools). Done = all 11 tools registered; `npm run build` passes; each tool callable via the MCP endpoint.
- [ ] Write `routines/process-brain.md`: prompt that polls `get_pending_work`, converts link/video sources via `convert_source` (Supadata), transcribes voice via Deepgram (note in prompt), reads images via Claude vision, then ingests each into the wiki via `ingest_source` following the brain's `CLAUDE.md` SOP. Done = prompt is self-contained with no placeholders; references the ac-rubicon MCP tools by name.
- [ ] Write `routines/brain-chat.md`: prompt that fetches the pending chat question, runs `search_wiki` (FTS) + `get_planning_context` for live app data, synthesizes an answer with `[title](/brain/wiki/slug)` citations, writes back via `save-answer`. Done = prompt self-contained; references ac-rubicon MCP tools.
- [ ] Write `routines/replan-day.md`: prompt that reads the two re-plan answers from the pending row, fetches current day plan + top tasks via MCP, regenerates remaining blocks, and saves via `save_day_plan`. Done = prompt self-contained; references ac-rubicon MCP tools.
- [ ] Update `routines/daily.md` (extends existing `plan-my-day.md` or replaces it): add a catch-up sweep step that checks `get_pending_work` after planning and notes any unprocessed captures. Done = prompt self-contained; backward-compatible with existing planner behavior.
- [ ] Write `routines/weekly.md`: prompt that runs `search_wiki` + `get_brain_pages` to lint wiki pages (missing overviews, stale dates, orphan pages) via `save_lint_report`, then generates a weekly insight card via `save_insight`. Done = prompt self-contained; references ac-rubicon MCP tools.
